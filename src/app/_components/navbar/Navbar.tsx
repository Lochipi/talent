...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header px-5 left-0 top-0 z-40 flex w-full items-center ${
          sticky
      ...
      >
        <div className="container">
          <div className="relative -mx-4 flex items-center justify-between">
            <div className="w-35 max-w-full ml-4 px-4 xl:mr-12">
              <Link
                href="/"
                className={`header-logo block w-full rounded-xl ${
                        ...
                        {menuItem.path ? (
                          <Link
                            ...
                            className={`flex py-2 text-base lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${
                              usePathName === menuItem.path
                                ? "text-gray-700"
                                : "text-dark hover:text-primary dark:hover:text-black"
                            }`}
                          >
                            {menuItem.title}
                          </Link>
                          ...
                          <>
                            <p
                              ...
                              className="text-black flex cursor-pointer items-center justify-between py-2 text-base group-hover:text-primary lg:mr-0 lg:inline-flex lg:px-0 lg:py-6"
                            >
                              {menuItem.title}
                              <span className="pl-3">
                            ...
                            >
                              {menuItem.submenu?.map((submenuItem, index) => (
                                <Link
                                  ...
                                  key={index}
                                  className="text-black block rounded py-2.5 text-sm hover:text-primary lg:px-3"
                                >
                                  {submenuItem.title}
                                </Link>
                              ))}
